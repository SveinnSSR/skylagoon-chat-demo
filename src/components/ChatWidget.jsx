...
const ChatWidget = ({ webhookUrl = 'https://sky-lagoon-chat-2024.vercel.app/chat', apiKey, language = 'en', isEmbedded = false, baseUrl }) => {
    ...
    const [processingSteps, setProcessingSteps] = useState([]); // Keep for compatibility, but we won't display steps
    ...
    // For thinking indicator, also force scroll
    useEffect(() => {
        if (isStreaming) {
            scrollToBottom(true);
        }
        ...
        switch (data.type) {
            ...
            case 'connected':
                ...
                break;
                            ...
                            case 'processingStep':
                    // Simply ignore processing steps - don't update or store them
                    console.log('[STREAMING] Processing step received but not displayed:', data.step);
                    break;
                    ...
                    break;
                            ...
                            case 'knowledge':
                    console.log('[STREAMING] Knowledge received:', data.count);
                    break;
                
                            case 'gpt_start':
                    console.log('[STREAMING] GPT processing started');
                    // Show thinking indicator through isStreaming state which is already true
                    break;
                ...
                setStreamFullText(prev => {
                    ...
                    // Check if this is the first chunk - if so, hide thinking indicator
                    if (prev === '') {
                        // First chunk arrived, the message is no longer just "thinking"
                        // but now actually showing content
                    }
                    
                    // Throttled scrolling for streaming - check for sentence completion
                    ...
                    // Use a batched update approach for typing messages for smoother experience
    ...
    // Extremely subtle thinking indicator 
            ...
            <div style={{
                ...
                {!isMinimized && (
                    <div style={{
                        ...
                        {/* Show traditional typing indicator if typing but not streaming */}
                        {isTyping && !isStreaming && (
                            <TypingIndicator />
                        )}
                        
                        {/* Show simplified thinking indicator for streaming instead of processing steps */}
                        {isStreaming && (
                            <ThinkingIndicator />
                        )}
